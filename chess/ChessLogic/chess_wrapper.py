    ...
    def _parse_fen(self, fen: str):
        ...
        try:
            ...
            for rank_idx, rank in enumerate(ranks):
                ...
                if file_idx != 8:
                    print(f"Invalid FEN: {fen}- Rank {rank_idx} has {file_idx} files, expected 8")
                    return
            ...
            if piece is not None and piece.color == self.turn:
                ...
                if piece.piece_type == PAWN:
                    ...
                    if 0 <= target_square < 64 and self.piece_at(target_square) is None:
                        moves.append(f"{SQUARE_NAMES[square]}{SQUARE_NAMES[target_square]}")
                    
                    # Pawns 2 in front in start
                    if (self.turn == WHITE and 48 <= square <= 55) or (self.turn == BLACK and 8 <= square <= 15):
                        target_square = square + (direction * 2)
                        if self.piece_at(target_square) is None and self.piece_at(square + direction) is None:
                            moves.append(f"{SQUARE_NAMES[square]}{SQUARE_NAMES[target_square]}")
                    
                    # Pawns attack diagonally
                    for capture_offset in [-9, -7] if self.turn == WHITE else [7, 9]:
                      
                        if (self.turn == WHITE and square % 8 == 0 and capture_offset == -9) or \
                           (self.turn == WHITE and square % 8 == 7 and capture_offset == -7) or \
                           (self.turn == BLACK and square % 8 == 0 and capture_offset == 7) or \
                           (self.turn == BLACK and square % 8 == 7 and capture_offset == 9):
                            continue
                            
                        target_square = square + capture_offset
                        if 0 <= target_square < 64:
                            target_piece = self.piece_at(target_square)
                            if target_piece is not None and target_piece.color != self.turn:
                                moves.append(f"{SQUARE_NAMES[square]}{SQUARE_NAMES[target_square]}")
                
                elif piece.piece_type == KNIGHT:
                    # Knights can move in L-shapes
                    knight_moves = [
                        square - 17, square - 15, square - 10, square - 6,
                    ...
                    for target in knight_moves:
                        if 0 <= target < 64:
                            ...
                            if abs((target % 8) - (square % 8)) <= 2:
                                ...
                                if target_piece is None or target_piece.color != self.turn:
                                    moves.append(f"{SQUARE_NAMES[square]}{SQUARE_NAMES[target]}")
                    ...
                    if piece:
                        ...
                        for square in SQUARES:
                            ...
                            if attacker and attacker.color != piece.color:
                                if abs((square % 8) - (to_square % 8)) <= 1 and abs((square // 8) - (to_square // 8)) <= 1:
                                    is_attacked = True
                                    break
    ...
    def detect_blunder(self, fen_before, fen_after, depth = 20):
        ...
        try:
            ...
            if eval_diff > BLUNDER_THRESHOLD:
                ...
                return True, f"Blunder! Best move was {best_move}. You lost {eval_diff/100:.1f} in evaluation."
            
            elif eval_diff > MISTAKE_THRESHOLD:
                print(f"MISTAKE detected! Difference: {eval_diff} centipawns")
                return True, f"Mistake! Best move was {best_move}. You lost {eval_diff/100:.1f} in evaluation."
            
            elif eval_diff > SMALL_MISTAKE_THRESHOLD:
                print(f"Small mistake detected: {eval_diff} centipawns")
                return True, f"Small mistake. Best move was {best_move}. You lost {eval_diff/100:.1f} in evaluation."
            else:
              
                if eval_diff < 0:
                    print(f"Good move! Improvement: {-eval_diff} centipawns")
                    return False, f"Good move! You gained {-eval_diff/100:.1f} in evaluation."
                else:
