        ...
        for entry in entries[:5]:
            ...
            if title is not None and link is not None:
                ...
                if published is not None and published.text:
                    try:
                        date_obj = datetime.strptime(published.text, "%Y-%m-%dT%H:%M:%SZ")
                        date_str = date_obj.strftime("%Y-%m-%d")
                    except:
                        pass
    ...
    try:
        if os.path.exists("chess_news.json"):
            if time.time() - os.path.getmtime("chess_news.json") < 3600:
                ...
                news_by_source = {
                    "chesscom_news": [article for article in all_news if article["source"] == "chess.com"],
                    "fide_news": [article for article in all_news if article["source"] == "fide.com"],
                    "lichess_news": [article for article in all_news if article["source"] == "lichess.org"],
                    "all_news": all_news
                }
